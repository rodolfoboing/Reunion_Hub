...
export default function ProfileScreen() {
    ...
    const saveProfile = async () => {
        ...
        // Fire-and-forget Firestore update (creates doc if not exists)
        const docRef = doc(db, 'users', auth.currentUser.uid);
        setDoc(docRef, {
            nick: editNick.trim(),
            bio: editBio,
            interests: editInterests,
            searchName: editNick.trim().toLowerCase(), // Helper for searching
            displayName: editNick.trim(), // Unify display name
        }, { merge: true }).catch(error => {
            console.error('Firestore sync error:', error);
        });

        // Update Auth Profile Display Name as well for consistency
        updateProfile(auth.currentUser, { displayName: editNick.trim() }).catch(e => console.log(e));
    ...
    return (
        <ScrollView style={styles.container} contentContainerStyle={styles.content}>
            <View style={styles.header}>
                <View style={[styles.avatar, userProfile?.photoURL && { backgroundColor: 'transparent' }]}>
                    {userProfile?.photoURL ? (
                        <TouchableOpacity>
                            <Image source={{ uri: userProfile.photoURL }} style={{ width: 100, height: 100, borderRadius: 50 }} />
                        </TouchableOpacity>
                    ) : (
